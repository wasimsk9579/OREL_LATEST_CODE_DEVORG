trigger FOC_Items_Inclusion on cgcloud__Order__c (after insert, after update) 
{
    System.debug('Trigger entered::');
    List<cgcloud__Order_Item__c> ItemList = [Select Id, cgcloud__Quantity__c, cgcloud__Product__c, cgcloud__Order__c from cgcloud__Order_Item__c where cgcloud__Order__c IN :Trigger.new AND FOC_added__c=false];
