...
retrieveQuoteAndInventoryData({ error, data }) {
    if (data) {
        ...
        this.initialQuoteLineItemsData = [...quoteLineItems];
...
handleQuantityChange(event) {
 // debugger;
  ...
  const updatedQuoteLineItems = this.initialQuoteLineItemsData.map((item, index) => {
      if (index === lineItemIndex) {
          const updatedItem = { ...item, Orl_Allocated_Quantity__c: event.target.value };
  ...
  console.log('Updated QuoteLineItems:', JSON.stringify(updatedQuoteLineItems));

  this.QuoteLineItemsData = updatedQuoteLineItems;
}
    ...
    //        console.log('change',JSON.stringify(this.QuoteLineItemsData));
    //        console.log('change',JSON.stringify(temp));
    ...
    //     if(event.target.value <= this.allocatevalue){ 
          
    //       console.log('temp after if',JSON.stringify(temp)); 
    //     debugger 
    //     temp[lineItemIndex].Orl_Allocated_Quantity__c=event.target.value;
          
    //    // this.initialQuoteLineItemsData = temp;
    //     console.log('temp144',JSON.stringify(temp));
    //       temp[lineItemIndex]['quantityExceeded']=false;  
    //     }else{
    //       temp[lineItemIndex]['quantityExceeded']=true;
    //     }
