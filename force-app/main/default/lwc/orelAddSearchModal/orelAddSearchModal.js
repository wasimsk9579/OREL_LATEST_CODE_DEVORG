...
export default class OrelAddSearchModal extends LightningElement{
    columns = [
    ...
    addProdHandler(event){
        console.log('add product button clicked::');
            console.log('advid::'+this.advid);
        console.log('JSON.stringify(this.orderItemsMap)::'+JSON.stringify(this.orderItemsMap));
        createOrderItems({itemsMap : JSON.stringify(this.orderItemsMap), orderId : this.advid})
        .then(result => {
            this.closeModal();
        }).catch(error => {
            console.log(error);
    ...
    closeModal(){
            ...
            //     bubbles : true,
            // });
            console.log('close modal in modal::')
    ...
    //     let dfValue = this.template.querySelector('lightning-datatable').draftValues[0];
